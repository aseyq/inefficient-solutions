...
# Plot
figure3 <- ggplot(df_cumulative, aes(x = period_in_chain, y = n_unique, color = treatment_appeal, group = interaction(generation, treatment_appeal))) +
    geom_line() +
    geom_point(size = 2) +
...
ggsave("output/figure3.png", plot = figure3, width = fig.width, height = fig.height, dpi = 300)
ggsave("output/figure3.pdf", plot = figure3, width = fig.width, height = fig.height, device = grDevices::cairo_pdf)
